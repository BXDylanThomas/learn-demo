...
import com.dylan.mybatisplusdruidview.controller.urlmapping.ColumnStrategy;
import com.dylan.mybatisplusdruidview.controller.urlmapping.MergeStrategy;
...
public class UrlMappingController {
            ...
            String join = Arrays.stream(method.getParameters()).filter(t->!filterClass.contains(t.getType()))
                    .map(x->{
                        ...
                        if (type == List.class ){
                            ...
                            typeName =x.getParameterizedType().getTypeName().substring(typeName.lastIndexOf(".")+1,typeName.indexOf(">"));
    ...
    private WriteHandler createMergeStrategy(int row1, int row2, int col1, int col2) {
        Map<Integer, MergeStrategy.Column> map = new HashMap<>();
        map.put(0,new MergeStrategy.Column(row1, row2, col1, col2));
        map.put(1,new MergeStrategy.Column(row1, row2, col1, col2));
        return new MergeStrategy(map);
    }
    ...
    private WriteHandler createColumnLengthStrategy(Map<String, List<String>> map) {
        ...
        int columnSize = map.values().stream().findFirst().map(List::size).orElse(0);
